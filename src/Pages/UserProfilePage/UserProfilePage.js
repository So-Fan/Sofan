...
function UserProfilePage({
  ...
  const [dataApiConcat, setDataApiConcat] = useState();
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < apiData.data.length; i++) {
      apiData.data[i].nftPriceEth = concatStringApiData(
        apiData?.data[i].nftPriceEth,
        7,
        7,
        false,
        false
    ...
    for (let i = 0; i < apiData.data.length; i++) {
      apiData.data[i].bid = concatStringApiData(
        apiData?.data[i].bid,
        7,
        7,
        false,
    ...
    if (isProfileSubMenuButtonClicked[0] === true) {
      return (
        <>
          ...
          />
          <NftCard
            userFromApiEth={dataApiConcat?.data[0].nftPriceEth}
            userFromApiBid={dataApiConcat?.data[0].bid}

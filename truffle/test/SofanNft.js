    ...
    context('setter and modifier', () => {
        ...
        describe('setter', () => {
          ...
          it("...should set sofan wallet to null", async () => {
            const storedData = await SofanNftInstance.SofanWallet({from:owner});           
            expect(storedData).to.equal(fourth); 
            await SofanNftInstance.setSofanWallet("0x0000000000000000000000000000000000000000")
            const storedData2 = await SofanNftInstance.SofanWallet({from:owner});  
            expect(storedData2).to.equal("0x0000000000000000000000000000000000000000"); 
          })
          it("...should withdraw rounded or excess amount of usdc", async () => {
            const tpm = await FiatTokenV2_1ViaProxy.allowance(owner, SofanNftInstance.address, {from:owner})
            console.log(new BN(tpm));
            await FiatTokenV2_1ViaProxy.transfer(SofanNftInstance.address, 1000000, {from:owner});
            const storedData = await FiatTokenV2_1ViaProxy.balanceOf(owner, {from:owner})
            expect(new BN(storedData)).to.be.bignumber.equal(new BN(10000000 - 1000000))
            const storedData2 = await FiatTokenV2_1ViaProxy.balanceOf(SofanNftInstance.address, {from:owner})
            expect(new BN(storedData2)).to.be.bignumber.equal(new BN(1000000))
            await SofanNftInstance.withdrawUSDC({from:owner})
            const storedData3 = await FiatTokenV2_1ViaProxy.balanceOf(SofanNftInstance.address, {from:owner})
            expect(new BN(storedData3)).to.be.bignumber.equal(new BN(0))
            const storedData4 = await FiatTokenV2_1ViaProxy.balanceOf(owner, {from:owner})
            expect(new BN(storedData4)).to.be.bignumber.equal(new BN(10000000))
          })
          it("...should be able to mint after active manually", async () => {
            let tempSofanInstance = await SofanNft.new(["CollectionName", "CollectionSymbol", "https://url.com/"], [3,100,1705063360,1000000], true, "0xd9145CCE52D386f254917e481eB44e9943F39138", 250, {from:owner});
            await tempSofanInstance.setAthleteWallet(third, {from:owner})
            await tempSofanInstance.setSofanWallet(fourth, {from:owner})
            await tempSofanInstance.setUsdcAddress(FiatTokenV2_1ViaProxy.address, {from:owner})
            await FiatTokenV2_1ViaProxy.approve(tempSofanInstance.address, 2000000, {from:owner})
            await expectRevert(tempSofanInstance.mint(owner, 1, 1000000, {from:owner}),"Cannot mint. Check Launchpad date !")
            await tempSofanInstance.activeMintManually({from:owner})
            await tempSofanInstance.mint(owner, 1, 1000000, {from:owner})
            const storedData = await tempSofanInstance.ownerOf(0, {from:owner})
            expect(storedData).to.equal(owner)
          })
          it("...should be able to set limit by wallet", async () => {
            ...
            const storedData2 = await SofanNftInstance.isLimitByWallet({from:owner})
            expect(storedData2).to.equal(true)
            const storedData3 = await SofanNftInstance.limitByWallet({from:owner})
            expect(new BN(storedData3)).to.be.bignumber.equal(new BN(1))
            await expectRevert(SofanNftInstance.mint(owner, 2, 2000000), "Limit of owned NFT reached or too much NFTs asked")
          })
          ...
          it("...should change base URI", async () => {
            await SofanNftInstance.setBaseURI("https://notsameurl.com/", {from:owner});
            const storedData = await SofanNftInstance.uri({from:owner})
